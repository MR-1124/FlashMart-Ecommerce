...
const Cart = () => {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <p className="text-dark-400 mb-8">{totalItems} item{totalItems !== 1 ? 's' : ''} in your cart</p>

      {cartItems.length === 0 ? (
        <div className="text-center py-20 glass-card">
          <HiOutlineShoppingBag className="w-16 h-16 text-dark-600 mx-auto mb-4" />
          <p className="text-dark-300 text-lg mb-2">Your cart is empty</p>
          <p className="text-dark-500 text-sm mb-6">Add some products to get started!</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-4">
            {cartItems.map((item) => (
              <div key={item.id} className="glass-card p-4 flex gap-4 items-center animate-fade-in">
                {/* Product Image */}
                <div className="w-20 h-20 bg-dark-700 rounded-xl flex-shrink-0 flex items-center justify-center">
                  <span className="text-2xl opacity-30">📦</span>
                </div>

                {/* Info */}
                <div className="flex-1 min-w-0">
                  <Link to={`/products/${item.product_id}`} className="text-white font-semibold hover:text-primary-400 transition-colors line-clamp-1">
                    {item.name}
                  </Link>
                  <p className="text-dark-400 text-sm">{item.category_name}</p>
                  <p className="text-white font-bold mt-1">{formatPrice(item.price)}</p>
                </div>

                {/* Quantity Controls */}
                <div className="flex items-center gap-1 bg-dark-800 rounded-lg">
                  <button
                    onClick={() => handleUpdateQuantity(item.id, Math.max(1, item.quantity - 1))}
                    disabled={item.quantity <= 1}
                    className="p-2 text-dark-400 hover:text-white disabled:opacity-30 transition-colors"
                  >
            ...
            <button onClick={handleClear} className="text-sm text-dark-400 hover:text-red-400 transition-colors">
              Clear entire cart
            </button>
          </div>
          ...
          <div className="lg:col-span-1">
            <div className="glass-card p-6 sticky top-24">
              ...
              <div className="space-y-3 border-b border-dark-700/50 pb-4 mb-4">
