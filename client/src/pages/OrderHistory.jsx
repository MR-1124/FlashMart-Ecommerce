...
const OrderHistory = () => {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <p className="text-dark-400 mb-8">Track and view your order history</p>

      {orders.length === 0 ? (
        <div className="text-center py-20 glass-card">
          <p className="text-4xl mb-4">📦</p>
          <p className="text-dark-300 text-lg mb-2">No orders yet</p>
          ...
          <div className="space-y-4">
            {orders.map((order) => (
              <Link
                key={order.id}
                to={`/orders/${order.id}`}
                className="glass-card p-5 block hover:border-primary-500/30 transition-all duration-200"
              >
